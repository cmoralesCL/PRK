...
export async function addHabitTask(values: Partial<Omit<HabitTask, 'id' | 'created_at' | 'archived_at'>>) {
    ...
    if (values.type === 'habit') {
        ...
        dataToInsert.frequency_unit = values.frequency_unit;
...
function isTaskActiveOnDate(task: HabitTask, date: Date): boolean {
    ...
    if (task.type === 'task' || task.type === 'project') {
        ...
        switch (task.frequency) {
            case 'daily':
                return true;
            case 'weekly': // Acumulativo
            case 'monthly': // Acumulativo
...
export async function getCalendarData(monthDate: Date) {
    ...
    const commitments = allHabitTasks.filter(task => {
        ...
        const totalProgressValue = logs.reduce((sum, log) => sum + (log.progress_value ?? (log.completion_percentage ? log.completion_percentage : 0)), 0);
    ...
    while(weekIndex < daysInView.length) {
        ...
        // 1. Daily tasks progress (already weighted by day)
        weekDays.forEach(d => {
        ...
        // 2. Weekly commitments progress (now with weight)
